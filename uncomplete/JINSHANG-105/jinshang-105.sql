INSERT INTO SHOW.PDGM_QueryAnalysis
SELECT sum(avgday),
       sum(netweight),
       sum(sheetnum),
       sum(kdwt),
       ...
       sum(byck)
FROM
  ( SELECT round(avg(sysdate-firstindate),0) avgday,
   ...
   WHERE r_record_create_date>=to_date(to_char(sysdate-1,'yyyy-mm-dd'),'yyyy-mm-dd')
   ...
   UNION ALL SELECT 0,
                    ...
                    0
   FROM wmsdba.t_zy_rk_inbill t
   WHERE r_record_create_date>=to_date(to_char(sysdate,'yyyy-mm'),'yyyy-mm')
     AND EXISTS
       (SELECT 1
        FROM WL.T_OWNER W
        WHERE SEG_NO='00103'
